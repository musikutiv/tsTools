...
grid | plot grid (default = TRUE)
with.average | plot average (default = FALSE)

## a simple example, one profile

I load the profile from the packages. For sake of space the profile just consists of one chromosome. Therefore I have to explictly re-create a SimpleRleList object. Typicall a Granges to coverage conversion would yield a SimpleRleList for direct usage.

The genomic annotations are taken from Bioconductors `TxDb.Dmelanogaster.UCSC.dm6.ensGene` library. Alternatively it can be created from GTF or GFF files using `GenomicFeatures` package.
